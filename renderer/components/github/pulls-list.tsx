...
export function PullsList() {
	...
	useEffect(() => {
		if (isAuthorizedError(error)) {
			clearAuthSession(useAuthStore.getState().setData).then(() => {
				router.replace("/auth");
			});
		}
	}, [error, router]);
	...
	const visibleRenderedPulls = renderedPulls.filter((entry) =>
		isPullVisible(entry.item, visibleLabels),
	);
	...
	const showNoData =
		!!pulls && pulls.items.length === 0 && visibleRenderedPulls.length === 0;
	const showFilteredEmpty =
		!!pulls &&
		pulls.items.length > 0 &&
		filteredPulls.length === 0 &&
		visibleRenderedPulls.length === 0;
