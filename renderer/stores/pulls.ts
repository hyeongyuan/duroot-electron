...
export const usePullsVisibleLabelsStore = create<PullsVisibleLabelsState>((set, get) => ({
  data: {},
  init: async () => {
...
export const filterVisibleLabels = (items: GithubIssueItem[] = [], visibleLabels: string[]) => {
  if (visibleLabels.length === 0) {
    return items;
  }

  return items.filter(item => item.labels.some(label => visibleLabels.includes(label.name)));
};
